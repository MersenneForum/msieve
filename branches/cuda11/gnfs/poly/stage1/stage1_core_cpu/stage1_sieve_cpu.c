...
stage1_bounds_update(msieve_obj *obj, poly_search_t *poly)
{
	...
	/* the CPU code's runtime is directly proportional
...
sieve_lattice_cpu(msieve_obj *obj, lattice_fb_t *L)
{
	...
	if (special_q_min == 1) { /* handle trivial case */
		...
		quit = sieve_lattice_cpu_core(obj, L, &sieve_special_q, 1, 1);
	}
	...
	while (1) {
		...
		quit = sieve_lattice_cpu_core(obj, L, &sieve_special_q,
						special_q_min2,
						special_q_max2);
