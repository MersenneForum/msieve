...
static const u32 multipliers[] = {1, 3, 5, 7, 
        11, 3*5, 3*7, 3*11, 
        5*7, 5*11, 7*11, 
        3*5*7, 3*5*11, 3*7*11, 
        5*7*11, 3*5*7*11 };
...
/*-----------------------------------------------------------------------*/
static u32 squfof_one_cycle(squfof_data_t *data, u32 mult_idx, 
...
u32 squfof(mpz_t n) {
  ...
  squfof_data_t data;
  static mpz_t scaledn, sqrt_scaledn;
  static u8 initialized = 0;

  if (initialized == 0) {
    mpz_init(scaledn);
    mpz_init(sqrt_scaledn);
    initialized = 1;
  }
  ...
  for (i = 0; i < MAX_MULTIPLIERS; i++) {
    ...
    mpz_mul_ui(scaledn, n, multipliers[i]);
    if (mpz_sizeinbase(scaledn, 2) > 62)
      break;

    mpz_root(sqrt_scaledn, scaledn, 2);

    /* initialize the rest of the fields for this
       multiplier */

    data.sqrtn[i] = mpz_get_ui(sqrt_scaledn);
    ...
    data.q1[i] = gmp2u64(scaledn) - (u64)data.p1[i] * data.p1[i];
