...
COMMON_SRCS = \
	...
	common/lanczos/lanczos_reorder.c \
...
all:
	...
	@echo "add 'METIS=1' for matrix reordering using Metis"
...
x86: $(COMMON_OBJS) $(QS_OBJS) $(QS_CORE_OBJS) \
	...
	ar r libmsieve.a $(COMMON_OBJS) $(QS_OBJS) \
			...
			$(NFS_OBJS)
	ranlib libmsieve.a
	cd metis && make CC="$(CC)" CFLAGS="$(CFLAGS)" && cd ..
	$(CC) $(CFLAGS) $(INC_FLAGS) demo.c -o msieve $(LDFLAGS) \
			libmsieve.a $(LIBS)
...
generic: $(COMMON_OBJS) $(QS_OBJS) $(QS_CORE_OBJS) \
		$(NFS_OBJS) $(GPU_OBJS)
	rm -f libmsieve.a
	ar r libmsieve.a $(COMMON_OBJS) $(QS_OBJS) \
...
mpqs/sieve_core_generic_32k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -DBLOCK_KB=32 -DCPU_GENERIC \
...
mpqs/sieve_core_generic_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -DBLOCK_KB=64 -DCPU_GENERIC \
		-DROUTINE_NAME=qs_core_sieve_generic_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_p2_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=pentium2 -DBLOCK_KB=64 \
		-DCPU_PENTIUM2 -DROUTINE_NAME=qs_core_sieve_p2_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_p3_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=pentium3 -DBLOCK_KB=64 \
		-DCPU_PENTIUM3 -DROUTINE_NAME=qs_core_sieve_p3_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_p4_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=pentium4 -DBLOCK_KB=64 \
		-DCPU_PENTIUM4 -DROUTINE_NAME=qs_core_sieve_p4_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_pm_32k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=pentium-m -DBLOCK_KB=32 \
		-DCPU_PENTIUM_M -DROUTINE_NAME=qs_core_sieve_pm_32k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_core_32k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=prescott -DBLOCK_KB=32 \
		-DCPU_CORE -DROUTINE_NAME=qs_core_sieve_core_32k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_k7_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=athlon -DBLOCK_KB=64 \
		-DCPU_ATHLON -DROUTINE_NAME=qs_core_sieve_k7_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_k7xp_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=athlon-xp -DBLOCK_KB=64 \
		-DCPU_ATHLON_XP -DROUTINE_NAME=qs_core_sieve_k7xp_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_k8_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=k8 -DBLOCK_KB=64 \
		-DCPU_OPTERON -DROUTINE_NAME=qs_core_sieve_k8_64k \
		-c -o $@ mpqs/sieve_core.c

mpqs/sieve_core_p4_64_64k.qo: mpqs/sieve_core.c $(COMMON_HDR) $(QS_HDR)
	$(CC) $(CFLAGS) $(INC_FLAGS) -march=nocona -DBLOCK_KB=64 \
		-DCPU_PENTIUM4 -DROUTINE_NAME=qs_core_sieve_p4_64k \
		-c -o $@ mpqs/sieve_core.c
