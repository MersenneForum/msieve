			...
			uint32 shift)
{
	...
	for (i = specialq_start; j == 0 && i < specialq_end; i++) {
		...
		if (i == specialq_start || curr_q->p != (curr_q-1)->p) {
			if (gcd64(p, curr_q->p) == 1)
				j = qq_prod = mod128_32(curr_q->pp, pp_v,
							pp_t, pp, pp_w);
		}
	...
	for (start_i = i - 1; i < specialq_end; i++) {
		...
		if (curr_q->p != (curr_q-1)->p) {
			if (gcd64(p, curr_q->p) == 1)
				j = qq_prod = montmul32(qq_prod, 
						mod128_32(curr_q->pp, pp_v,
							pp_t, pp, pp_w), 
						pp, pp_w);
			else
				j = 0;
	...
	for (i--; (int32)i >= (int32)start_i; 
		...
		inv = montmul32(inv, mod128_32(q_batch[q].pp, pp_v, pp_t,
						pp, pp_w), pp, pp_w);

		do {
			qroot = mod128_32(q_batch[q].root, pp_v,
							pp_t, pp, pp_w);
	...
	while ((int32)q >= (int32)start_i) {
		...
		qroot = mod128_32(q_batch[q].root, pp_v, pp_t, pp, pp_w);
			...
			uint32 shift)
{
	...
	uint64 pp, pp_v, pp_t, qq_prod, qroot, newroot, write_val;
	uint32 specialq_start, specialq_end;
	uint32 q, qq_prod_offset, curr_offset, q_count;
	uint32 i, j, k, m, start_i; 
	uint64 inv, curr_inv;
	specialq_t *curr_q;
	...
	for (i = specialq_start; write_val == 0 && i < specialq_end; i++) {
		...
		if (i == specialq_start || curr_q->p != (curr_q-1)->p) {
			if (gcd64(p, curr_q->p) == 1)
				write_val = qq_prod = mod128_64(curr_q->pp,
						pp_v, pp_t, pp, pp_w);
		}
	...
	for (i--; (int32)i >= (int32)start_i; 
		...
		inv = montmul64(inv, mod128_64(q_batch[q].pp, pp_v, pp_t,
						pp, pp_w), pp, pp_w);

		do {
			qroot = mod128_64(q_batch[q].root, pp_v,
							pp_t, pp, pp_w);
	...
	while ((int32)q >= (int32)start_i) {
		...
		qroot = mod128_64(q_batch[q].root, pp_v, pp_t, pp, pp_w);
